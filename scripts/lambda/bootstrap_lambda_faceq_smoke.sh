#!/usr/bin/env bash
# Bootstrap a Lambda Cloud instance and launch the FACE-Q 8x FSDP smoke.
#
# Inputs are read from the Lambda poller launch flag by default. Secrets are
...
OUT_DIR="${OUT_DIR:-.codex_outputs/lambda_scale_20260517}"
LAUNCH_FLAG="${LAUNCH_FLAG:-$OUT_DIR/lambda_launched.flag}"
...
RUN_STAMP="${RUN_STAMP:-$(date -u +%Y%m%d_faceq_1p4b_fsdp_lambda_smoke)}"
REMOTE_LAB_ROOT="${REMOTE_LAB_ROOT:-/tmp/clearmesh_faceq_lambda_scale_train_$RUN_STAMP}"
REMOTE_LOG="${REMOTE_LOG:-/tmp/clearmesh_faceq_lambda_scale_train.nohup.log}"
REMOTE_PID="${REMOTE_PID:-/tmp/clearmesh_faceq_lambda_scale_train.pid}"
...
mkdir -p "$OUT_DIR"
BOOTSTRAP_STARTED="$OUT_DIR/lambda_bootstrap_started.flag"
BOOTSTRAP_DONE="$OUT_DIR/lambda_bootstrap_done.flag"
LOCAL_LOG="$OUT_DIR/lambda_bootstrap.log"

if [[ ! -f "$LAUNCH_FLAG" ]]; then
  echo "Missing launch flag: $LAUNCH_FLAG" >&2
  exit 2
fi
if [[ ! -f "$LAMBDA_ENV_FILE" ]]; then
  echo "Missing Lambda env file: $LAMBDA_ENV_FILE" >&2
  exit 2
fi
...
INSTANCE_ID="$(python3 - "$LAUNCH_FLAG" <<'PY'
import json
import sys
...
if [[ -z "$INSTANCE_ID" ]]; then
  echo "Could not parse Lambda instance id from launch flag." >&2
  exit 2
fi

lambda_get_instances() {
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Waiting for Lambda instance $INSTANCE_ID..." | tee -a "$LOCAL_LOG"
INSTANCE_JSON="$OUT_DIR/lambda_instance.latest.json"
deadline=$(( $(date +%s) + 1800 ))
REMOTE_HOST=""
while [[ "$(date +%s)" -lt "$deadline" ]]; do
  lambda_get_instances > "$INSTANCE_JSON.tmp"
  mv "$INSTANCE_JSON.tmp" "$INSTANCE_JSON"
  REMOTE_HOST="$(python3 - "$INSTANCE_ID" "$INSTANCE_JSON" <<'PY'
import json
import sys
...
for item in payload.get("data", []):
    if str(item.get("id")) == target:
        ...
        if status in {"active", "running", "booted"} and ip:
            print(ip)
        raise SystemExit
PY
)"
...
if [[ -z "$REMOTE_HOST" ]]; then
  echo "Lambda instance did not become SSH-ready before timeout." >&2
  exit 3
fi
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Preflighting Lambda GPU host..." | tee -a "$LOCAL_LOG"
ssh "${SSH_OPTS[@]}" "$SSH_TARGET" 'hostname; nvidia-smi; mkdir -p /home/ubuntu/clearmesh' | tee -a "$LOCAL_LOG"

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Syncing repository to Lambda..." | tee -a "$LOCAL_LOG"
tar \
  --exclude='.git' \
...
remote_script="$OUT_DIR/remote_lambda_faceq_smoke.sh"
...
if [[ ! -x "\$VENV/bin/python" ]]; then
  python3 -m venv "\$VENV" || (sudo apt-get update && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y python3-venv && python3 -m venv "\$VENV")
fi
...
scp "${SSH_OPTS[@]}" "$remote_script" "$SSH_TARGET:/tmp/clearmesh_lambda_faceq_smoke.sh" >/dev/null
ssh "${SSH_OPTS[@]}" "$SSH_TARGET" "chmod +x /tmp/clearmesh_lambda_faceq_smoke.sh && nohup /tmp/clearmesh_lambda_faceq_smoke.sh > '$REMOTE_LOG' 2>&1 & echo \\$! > '$REMOTE_PID'"

cat > "$OUT_DIR/lambda_run_info.json" <<JSON
{
  "provider": "lambda",
...
JSON
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Lambda FACE-Q smoke launched instance=$INSTANCE_ID host=$REMOTE_HOST root=$REMOTE_LAB_ROOT" | tee -a "$LOCAL_LOG"
cp "$OUT_DIR/lambda_run_info.json" "$BOOTSTRAP_DONE"
