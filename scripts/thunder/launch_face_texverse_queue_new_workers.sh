...
for worker_index in $(seq 0 $((WORKER_COUNT - 1))); do
  ...
  (
    ...
    CREATE_INSTANCE=1 \
      ...
      PAPER_WITHIN_FACE_ORDER="${PAPER_WITHIN_FACE_ORDER:-rotate_min_zyx}" STRICT_ENGINE="${STRICT_ENGINE:-voxel_shell}" FALLBACK="${FALLBACK:-convex_hull}" TEST_RATIO="${TEST_RATIO:-0.02}" \
