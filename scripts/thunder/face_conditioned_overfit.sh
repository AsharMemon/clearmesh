...
RUN_DIR="${RUN_DIR:-/tmp/clearmesh_face_conditioned_overfit}"
STEPS="${STEPS:-800}"
...
python scripts/research/train_face_conditioned_tiny.py \
  ...
  --condition-tokens 4 \
  --lr 5e-4 \
  --output "$RUN_DIR/face_conditioned_overfit.pt"
python scripts/research/sample_face_conditioned_tiny.py \
  --checkpoint "$RUN_DIR/face_conditioned_overfit.pt" \
